...
class GetDripAPI(object):
        ...
        if self.auth == "auth_header":
            ...
            if payload:
                response = requests.post(
                    ...
                    auth=self.user_passwd,
                    data=json.dumps(payload),
                )
            else:
                response = requests.post(
                    url, headers=self.headers, auth=self.user_passwd
                )
        return response.status_code, response.json()
    ...
    def api_post_params(self, url, params=None):
        ...
        if self.auth == "auth_header":
            ...
            response = requests.post(
                url, headers=self.headers, auth=self.user_passwd, params=params
            )
        return response.status_code, response.json()
    ...
    def subscribe_subscriber(self, campaign_id, payload):
        ...
        url = "%s/%s/campaigns/%s/subscribers" % (
            self.api_url,
            self.account_id,
            campaign_id,
        )
    ...
    def campaign_subscriber(self, subscriber_id):
        ...
        url = "%s/%s/subscribers/%s/campaign_subscriptions" % (
            self.api_url,
            self.account_id,
            subscriber_id,
        )
